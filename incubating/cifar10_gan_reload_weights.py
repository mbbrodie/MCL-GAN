...
def cifar10quick(inputs):
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                    ...
                    activation_fn=None):
        net = slim.conv2d(inputs, 32, [5, 5], stride=1, padding='SAME', weights_initializer=tf.random_normal_initializer(stddev=0.001), scope='conv1')
        net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool1')
        net = tf.nn.relu(net)
        net = slim.conv2d(net, 32, [5, 5], stride=1, padding='SAME', weights_initializer=tf.random_normal_initializer(stddev=0.01), scope='conv2')
        net = tf.nn.relu(net)
        net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool2')
        net = slim.conv2d(net, 64, [5, 5], stride=1, padding='VALID', weights_initializer=tf.random_normal_initializer(stddev=0.01), scope='conv3')
        net = tf.nn.relu(net)
        net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool3')
        #net = slim.stack(net, slim.fully_connected, [64, 10], weights_initializer=tf.random_normal_initializer(stddev=0.1), scope='fc')
        predictions = slim.stack(net, slim.fully_connected, [64, 10], weights_initializer=tf.random_normal_initializer(stddev=0.1), scope='fc')
        ...
        with tf.Session() as sess:
            ...
            for step in xrange(50):
                ...
                if step % 100 == 0:
                    ...
                    print('loss_m1', 1.0*sum(batch_loss_m1) / (self.batch_size + step))
                    print('loss_m2', 1.0*sum(batch_loss_m2) / (self.batch_size + step))
                    if step > 3000 and step % 10 == 0:
                        print('loss_unique', 1.0*sum(batch_unique_loss) / (self.batch_size + step))
            
                train_idx = train_idx + self.batch_size
                #if self.batch_size + train_idx * self.batch_size > 50000:
